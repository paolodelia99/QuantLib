...
namespace {
	bool string_view_starts_with(std::string_view sv, std::string_view prefix) {
...
const Handle<Quote> FxIndex::fxQuote(bool withSettlementLag) const {
    ...
    if (quote.empty()) {
        if (fxRate_.empty()) {
            ...
            if (!useQuote_)
                tmpQuote = Handle<Quote>(ext::make_shared<SimpleQuote>(
            ...
            fxRate_ = Handle<Quote>(
                ext::make_shared<FxRateQuote>(tmpQuote, sourceYts_, targetYts_, fixingDays_, fixingCalendar_));
        }
        quote = fxRate_;
    }
    return quote;
...
ext::shared_ptr<FxIndex> FxIndex::clone(const Handle<Quote> fxQuote, const Handle<YieldTermStructure>& sourceYts,
    ...
    return ext::make_shared<FxIndex>(famName, fixingDays_, sourceCurrency_, targetCurrency_, fixingCalendar_, quote,
                                       source, target);
}
...
Real FxIndex::pastFixing(const Date& fixingDate) const {
    ...
    if (fixingTriangulation_) {
        ...
        for (auto i : availIndexes) {
            if (string_view_starts_with(i, familyName_)) {
                // check for a fixing
