...
VannaVolgaSmileSection::VannaVolgaSmileSection(Real spot, Real rd, Real rf, Time t, Volatility atmVol, Volatility rr,
    ...
    try {
        BlackDeltaCalculator atmCalc(Option::Type::Call, deltaType, spot, domesticDiscount(), foreignDiscount(),
                               sqrt(t) * atmVol);
        k_atm_ = atmCalc.atmStrike(atmType);
    ...
    try {
        BlackDeltaCalculator callCalc(Option::Type::Call, deltaType, spot, domesticDiscount(), foreignDiscount(),
                               sqrt(t) * vol_c_);
        k_c_ = callCalc.strikeFromDelta(delta);
    } catch (const std::exception& e) {
    ...
    try {
        BlackDeltaCalculator putCalc(Option::Type::Put, deltaType, spot, domesticDiscount(), foreignDiscount(),
                               sqrt(t) * vol_p_);
        k_p_ = putCalc.strikeFromDelta(-delta);
    } catch (const std::exception& e) {
...
Volatility VannaVolgaSmileSection::volatility(Real k) const {
    ...
    const Real logk2k1 = log(k2 / k1);
    const Real logk3k1 = log(k3 / k1);
    const Real logk3k2 = log(k3 / k2);
    Real r1 = log(k2 / k) * log(k3 / k) / (logk2k1 * logk3k1);
    Real r2 = log(k / k1) * log(k3 / k) / (logk2k1 * logk3k2);
    Real r3 = log(k / k1) * log(k / k2) / (logk3k1 * logk3k2);
